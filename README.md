...
纯图片默认等待 `weixin.image_merge_window_sec=10` 秒：同一用户随后发送的文字会作为图片说明合并，继续发送图片会重新计时；超时后才使用默认图片分析提示。设为 `0` 可关闭等待。入站图片按 Gateway 的完整微信会话键保存，因此 `ask_image` 使用无扩展名的 `image_id` 即可解析对应文件。

Bridge stdout 只承载 JSONL 协议，敏感凭据、cursor 和 context token 始终留在权限为 `0700/0600` 的状态目录；Python Channel 只在同一目录原子保存不含凭据的待合并图片元数据，以便进程重启后继续投递。若收到 `-14` 会话失效，Bridge 会停止轮询、清除当前凭据代次的 context/去重状态并要求重新扫码；重新登录后，对端需再次交互才能恢复主动发送。当前真实微信扫码/收发属于有外部副作用的手工验收，不在自动测试中执行。

主动提醒启用后，可在允许的微信私聊中发送 `/bind-reminders`。一个提醒数据库同一时刻
只绑定一个飞书或微信私聊。当前用户主动发送 `/unbind-reminders` 会释放绑定但保留已有
任务；随后可在另一渠道或由另一用户发送 `/bind-reminders`，原任务整体迁移到新目标。
静态提醒和到点运行 Agent 的动态任务都复用现有调度器。微信提醒使用稳定 execution
correlation ID 和 Bridge 持久 context；若登录代次失效或 context 丢失，系统只会暂停
并保留原所有权，重新扫码后由同一用户发送 `/bind-reminders` 即可恢复。需要换绑时必须
先由原用户主动解绑，掉线不会自动开放其它用户接管。
...
主动提醒首次使用时，在希望接收提醒的飞书或微信私聊中发送 `/bind-reminders`。
绑定成功后，可以从飞书、微信、Web 或 CLI 用自然语言让 Agent 创建、查询或取消任务，
任务始终发送到当前唯一绑定私聊。
`message` 任务到点直接发送创建时写好的正文，`agent` 任务到点才运行
独立 Agent 会话获取实时内容。原绑定用户发送 `/unbind-reminders` 会释放目标但保留
任务，随后可以在飞书或微信重新绑定，已有任务整体跟随新目标。系统因微信登录或 context
失效触发的自动暂停不会释放所有权，只允许原用户恢复；绑定命令不接受群聊。

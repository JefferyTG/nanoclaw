...
主动提醒启用后，可在允许的微信私聊中发送 `/bind-reminders`。首次成功绑定会把该
提醒数据库永久锁定给当前微信渠道和用户；V1 中飞书与微信二选一，`/unbind-reminders`
只暂停并保留任务，不释放所有权，也不允许另一渠道接管。绑定后可直接用自然语言创建、
查询或取消提醒；静态提醒和到点运行 Agent 的动态任务都复用现有调度器。微信提醒使用
稳定 execution correlation ID 和 Bridge 持久 context；若登录代次失效或 context 丢失，
目标会暂停，重新扫码并由同一用户再次发送 `/bind-reminders` 后恢复。
...
首次绑定的渠道和用户成为该实例的唯一永久目标；绑定成功后，可以从飞书、微信、Web
或 CLI 用自然语言让 Agent 创建、查询或取消任务，任务始终发送到该已绑定私聊。
`message` 任务到点直接发送创建时写好的正文，`agent` 任务到点才运行
独立 Agent 会话获取实时内容。发送 `/unbind-reminders` 会暂停目标调度但保留任务，
同一渠道的同一用户重新绑定后恢复。绑定命令不接受群聊，一个实例也不能改绑给另一
渠道或用户。

...
| 记忆体系 | ✅ | USER/MEMORY/HISTORY/daily、SQLite LIKE 检索、上下文预算动态配置（`context_budget_tokens`，默认 512k）超预算压缩（`ContextCompactor`，每会话独立实例、压缩不写 daily，TASK-006）、跨会话记忆同步（全局/会话 revision + `<memory_patch>` 补丁注入与持久化；压缩后无条件重建完整快照，TASK-007；摘要输入降噪——工具结果只留结论，TASK-008；分块结构化摘要——map-reduce 按字段组织，TASK-009） |
...
| 模块 | 职责 | 关键文件 |
|---|---|---|
| `main.py` | 唯一 composition root：装配共享对象、渠道、生命周期、信号处理 | main.py |
...
- 当前里程碑：TASK-001~009 已完成并归档（任务卡见 `docs/tasks/completed/`）；暂无 active 任务。

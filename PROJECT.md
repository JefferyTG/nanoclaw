...
| 网页端 | ✅ | 流式思考/逐字输出、会话侧边栏、历史接回/删除、断线重连、「⏹ 停止」回合取消；**移动端适配**（TASK-039）：`@media 768px` 响应式（桌面零影响）、侧边栏抽屉化+汉堡开合+遮罩防误触、100dvh 兜底 iOS 软键盘不顶飞、输入框 16px 防缩放、safe-area-inset-bottom、按钮 ≥44px 触控区、配置面板窄屏单列 |
...
- 当前里程碑：TASK-001~039 已完成并归档（TASK-038 AEC 实验已回退并归档，任务卡均在 `docs/tasks/completed/`）；active：TASK-040 webui 多端历史同步（待开始，乖宝 08-12 实测多端发现）。任务卡在 `docs/tasks/active/`。

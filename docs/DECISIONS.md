...
| webui 实时通话下行 PCM 直通（TASK-044 决策） | 有效 | `client.py::create_session` 输出写死 `pcm_s16le` 24k，与 demo 的 `ogg_opus` 不一致；走 opus 需 Python 端加编码或搬 demo 的 WebCodecs/MediaSource 解码，改动大且低端安卓兼容风险高。选 PCM 直通：下行 base64 → 浏览器 AudioBuffer 播 int16 PCM（≈48KB/s，局域网无压力），砍掉 demo 的 OggOpusDemuxer/WebCodecsOggOpusPlayer/MediaSourceEncodedAudioPlayer。符合 ponytail：先跑起来，不搬复杂解码 |
| webui 实时通话并发互斥（TASK-044 决策） | 有效 | 一次只允许一路实时通话（方案 A，乖宝 2026-08-13 确认）：`WebChannel._active_realtime` 互斥位在 web 事件循环内 check 与 set 之间无 await（单线程原子），第二路 `/api/realtime` 返回 `realtime.error(code=busy)` 并关闭，不踢旧通话。若未来迁多 worker 进程需改跨进程互斥 |
| web 流事件按会话广播（TASK-046 决策） | 有效 | 流式事件原按 conn_id 单发，断线即丢。改为 `StreamEvent`/`OutboundMessage` 带 `session_key`（web=current_key，Gateway 取 `msg.sender_id`），`WebChannel` 维护 `_key_conns`（会话→连接集合，open/new 绑定、断开解绑），`stream_event` 带 key 时广播给集合内所有存活连接（查不到退回单发），`send` 发起连接断开时按 key 转发。前端：断线保留聊天区、重连自动 open + 按 active 判定（在跑→保留渲染续流 / 已完→全量重渲染补拉）、主界面任务横幅、历史工具卡片还原（仅展示）、新开页面自动接管活跃会话。附带打通 TASK-040「事件按会话广播」核心。取舍：断网期间中间事件（thinking/tool 过程）不可补（数据源未落盘），乖宝确认接受（B 方案：中间过程省略，结果自动出现）。验证：984 tests 全绿 + 真机多轮断线重连验收 |
...
| realtime 渠道不走消息总线（TASK-037 决策） | 有效 | 豆包 S2S 端到端全双工（语音进→语音出、对话思考在服务端内部），`RealtimeChannel` 继承 `Channel` 但 `bus=None`、`send()` 空实现，只复用 start/stop 生命周期；独立闭环不经过 Gateway / AgentLoop / 记忆 |

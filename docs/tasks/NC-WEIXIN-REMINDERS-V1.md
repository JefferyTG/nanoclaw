...
- 执行子 Agent：`reminder_target_core`、`reminder_scheduler_defer`、
  `weixin_reminder_binding`（均为 gpt-5.6-terra / medium）
...
- 一个提醒数据库只有一个永久所有目标。首次成功 `/bind-reminders` 的
  `(channel, owner_id)` 锁定该实例；飞书与微信二选一。
- 同一 owner 可在同一渠道重绑；`/unbind-reminders` 仅暂停并保留任务，不释放所有权。
- 跨渠道切换、双投递、群聊、多账号、提醒图片/语音/文件不属于 V1。
- 工具不接受 channel、chat/user ID 或 token；Web/CLI/任意聊天会话仍统一操作唯一目标。
- 不新增配置项；微信绑定继续受 `weixin.allowed_user_ids` 约束。

### 共享目标契约

- 领域字段：`channel`、`recipient_id`、`owner_id`、`active`；稳定公开 ID 仍为 UUID
  `target_id`，任务外键不变。
...
- `bind_target(channel, recipient_id, owner_id, now_utc)`：首次建立 owner；仅相同
  channel + owner 可重绑，其他情况抛 `PermissionError`。
- `unbind_target(channel, owner_id, now_utc)`：仅所有者可暂停。
...
- `suspend_target(target_id, now_utc)`：微信 `session_expired/context_missing` 时暂停目标；
  用户重新扫码并以同一身份发送 `/bind-reminders` 后恢复并唤醒 Scheduler。
...
- 关键决策与假设：见上方共享契约；首次成功绑定永久锁定渠道与 owner。

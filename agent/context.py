    ...
    def _channel_section(self) -> str:
        ...
        if self.channel == "weixin":
            base += (
                ...
                "③ 对方连发多条消息（换行分隔合并送达）时，视为一口气的碎碎念——"
                "先接住最新一条，再自然回应前面的，不要机械地「你说了 1、2、3 点」。"
            )
        if self.channel == "voice":
            # voice 渠道：本地语音对讲（TASK-027 连续对讲）。sender_id 是
            # local:<seq>（多会话序号），与渠道专属行为无关，故分支只看渠道名。
            base += (
                "【voice 连续对讲模式】本地语音对讲渠道（语音播报听不了长篇大论）：\n"
                "① 回复要简短俏皮口语化——一两句话说完，别整长篇大论；除非用户问的是"
                "知识类/需要详细解释的问题（如历史、科普、代码讲解），可以适当多讲；\n"
                "② 甜度保持小奈风格，但句子短、有节奏，适合口头播报；\n"
                "③ 回复必须用纯口语文字：不要用 markdown 标记（**加粗**、`代码`、"
                "# 标题、- 列表）、不要用 emoji 或颜文字、不要用奇怪符号（→★☆◆ 等）"
                "——要表达开心就说「哈哈」「嘿嘿」，难过就说「呜呜」「唉」，"
                "这些符号被 TTS 语音播报会念得很奇怪；\n"
                "④ 用户告别/结束话题（如拜拜、不聊了、晚安、先这样吧等）时，回复末尾"
                "加一行结束语标记 [END]，渠道会把它剥离并自动退出连续对话回待唤醒。"
            )
        return base
